...
| `project-manager-mode` | ユーザーが product 側、Claude が project manager という分業で働く。調査は読み取り専用サブエージェントに本体ディレクトリで、修正は worktree を切って実装役に委譲。件ごとに「決裁が要るか」を判定し、要らないものは全体のまとめを待たず単独で直し、要るものは実装を進めず「それ単体で決められる形」に整えて返す。完了時は未実施の検証まで含めた 5 点で報告する |
